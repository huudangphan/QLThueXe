...
namespace QLThueXe.Services.Repository
{
    public class XeRepository
    {
        ...
        public string SearchXe(string bien_so, int id_hang, string ten_xe, double gia_thue, string ma_ks,bool status)
        {
            ...
            string query = string.Format("BEGIN TRANSACTION; SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; select * from search_xe('{0}',{1},'{2}',{3},'{4}',{5}) ", bien_so, id_hang, ten_xe, gia_thue, ma_ks, status);
        ...
        {
            string query = string.Format(" BEGIN TRANSACTION; SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; select * from update_xe('{0}',{1},{2}) ", bien_so, gia_thue, status);
            string query2 = "commit;";
